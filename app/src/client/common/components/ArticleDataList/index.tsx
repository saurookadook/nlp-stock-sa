...
function ArticleAccordianHeading({
  articleRecord,
}: React.PropsWithChildren<{ articleRecord: ArticleDataEntry }>) {
  return (
    <Heading>
      <AccordionButton>
        <Box
...
function ArticleDataList({
  articleData,
}: React.PropsWithChildren<{ articleData: ArticleDataEntry[] }>) {
  return (
    // TODO: maybe this should use `ul` and `li` elements?
    <Accordion allowMultiple>
      {articleData.map((record) => (
        <ArticleDataListItem
          key={`article-record-${record.id}`}
